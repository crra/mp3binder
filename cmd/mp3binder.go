...
func addXingHeader(file string, totalFrames, totalBytes uint32) {
	outputFile, err := ioutil.TempFile(filepath.Dir(file), "*")
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(1)
	}
	...
	// Swap
	err = os.Rename(outputFile.Name(), file)
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(1)
	}
...
func addID3v2Tag(mp3Path, tagPath string) {
	...
	if id3tag != nil {
		outputFile, err := ioutil.TempFile(filepath.Dir(mp3Path), "*")
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			os.Exit(1)
		}
		...
		err = os.Rename(outputFile.Name(), mp3Path)
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			os.Exit(1)
		}
